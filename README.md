...
### 项目介绍
处方共享区块链是一个非常实用的应用，可以广泛应用于互联网医院、分级转诊、处方外流等诸多医疗应用场景。区块链上的电子处方是加密的，有数字签名且不可篡改的，因此，具有不可抵赖和隐私保护的天然特质。以电子处方为代表的个人健康数据隐私计算的应用和普及将改善老百姓的就医体验，影响每一个人的健康生活，彻底改变人们的生活方式。

### 应用场景
- 互联网医院 - 互联网医院可以通过”处方共享区块链“为患者开电子处方，患者在身边药房就近买药
- 分级转诊 - 医联体之间医疗数据共享，三甲医院和社区医院之间的数据共享。
- 处方外流 - 以城市或者地区为单位成立跨区域医疗信息协作共享平台。1.医生在医院为患者开处方，2.患者接受电子处方(在链上)，3.患者在就近药店买药(包括处方药)，4.药店核验处方真伪，药店依据处方售药。5.留存备案/政府监管

### 低层技术
 - Cosmos SDK - 完全自主可控的区块链，可独立部署，不依赖与任何其他公链，成本可控。可扩展性强，既可以作为联盟链运营，也可作为公链运营，为项目实施落地方案提供更多选择。
 - 加密算法 - 默认采用国际领先的加密算法,来先实现秘钥交换和数据分组加密。可以根据需要选用中国国家标准加密算法: SM2和SM4
